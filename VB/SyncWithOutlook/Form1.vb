...
Namespace SyncWithOutlook
    ...
    Public Partial Class Form1
        ...
        Private optionsForm As SyncronizationOptionForm = New SyncronizationOptionForm()
        ...
        ' Scheduler --> Outlook
        Private Sub btnSchedulerOutlook_Click(ByVal sender As Object, ByVal e As EventArgs)
...
'#Region "#scheduler->outlook"
                ...
                AddHandler synchronizer.AppointmentSynchronizing, New AppointmentSynchronizingEventHandler(AddressOf exportSynchronizer_AppointmentSynchronizing)
                ' The AppointmentSynchronized event indicates that the operation for a particular appointment is complete.
                AddHandler synchronizer.AppointmentSynchronized, New AppointmentSynchronizedEventHandler(AddressOf exportSynchronizer_AppointmentSynchronized)
                ' Specify MS Outlook calendar path.
                CType(synchronizer, ISupportCalendarFolders).CalendarFolderName = comboBoxEdit1.EditValue.ToString()
                ' Perform the operation.
                synchronizer.Synchronize()
'#End Region  ' #scheduler->outlook
                memoEdit1.Text += Microsoft.VisualBasic.Constants.vbCrLf & " Export is done." & Microsoft.VisualBasic.Constants.vbCrLf & Microsoft.VisualBasic.Constants.vbCrLf
            Else
        ...
        Private Sub exportSynchronizer_AppointmentSynchronizing(ByVal sender As Object, ByVal e As AppointmentSynchronizingEventArgs)
            AnalyzeAndHandleCurrentOperation(TryCast(e, OutlookAppointmentSynchronizingEventArgs), False)
        End Sub

        ' Outlook --> Scheduler
        Private Sub btnOutlookScheduler_Click(ByVal sender As Object, ByVal e As EventArgs)
            If comboBoxEdit1.EditValue IsNot Nothing Then
'#Region "#outlook->scheduler"
                ...
                synchronizer.ForeignIdFieldName = OutlookEntryIDFieldName
                ' The AppointmentSynchronizing event allows you to control the operation for an individual appointment.
                AddHandler synchronizer.AppointmentSynchronizing, New AppointmentSynchronizingEventHandler(AddressOf importSynchronizer_AppointmentSynchronizing)
                ' The AppointmentSynchronized event indicates that the operation for a particular appointment is complete.
                AddHandler synchronizer.AppointmentSynchronized, New AppointmentSynchronizedEventHandler(AddressOf importSynchronizer_AppointmentSynchronized)
                ' Specify MS Outlook calendar path.
                CType(synchronizer, ISupportCalendarFolders).CalendarFolderName = comboBoxEdit1.EditValue.ToString()
                ' Perform the operation.
                synchronizer.Synchronize()
'#End Region  ' #outlook->scheduler
                memoEdit1.Text += Microsoft.VisualBasic.Constants.vbCrLf & " Import is done." & Microsoft.VisualBasic.Constants.vbCrLf & Microsoft.VisualBasic.Constants.vbCrLf
            Else
                MessageBox.Show("Please select an Outlook folder.", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            End If
        End Sub
        ...
        Private Sub importSynchronizer_AppointmentSynchronizing(ByVal sender As Object, ByVal e As AppointmentSynchronizingEventArgs)
            AnalyzeAndHandleCurrentOperation(TryCast(e, OutlookAppointmentSynchronizingEventArgs), True)
        End Sub
        ...
        Private Sub AnalyzeAndHandleCurrentOperation(ByVal eventArgs As OutlookAppointmentSynchronizingEventArgs, ByVal toScheduler As Boolean)
            ...
            If eventArgs.Appointment IsNot Nothing Then
                appointmentInfoScheduler = String.Format("Subject: {0}, Start: {1}, End: {2}, ForeignID: {3}", eventArgs.Appointment.Subject, eventArgs.Appointment.Start.ToString(), eventArgs.Appointment.End.ToString(), eventArgs.Appointment.GetValue(schedulerStorage1, OutlookEntryIDFieldName))
            Else
            ...
            Select Case eventArgs.Operation
                Case SynchronizeOperation.Create
                    ...
                    logInfo += Microsoft.VisualBasic.Constants.vbCrLf
            ...
            memoEdit1.Text += logInfo & Microsoft.VisualBasic.Constants.vbCrLf
        End Sub
...
'#Region "Initialization"
        Private Sub InitAppointments()
            Dim aptmappings As AppointmentMappingInfo = schedulerStorage1.Appointments.Mappings
            ...
            Dim resmappings As ResourceMappingInfo = schedulerStorage1.Resources.Mappings
        ...
        Private Sub GenerateResources(ByVal number As Integer)
            For i As Integer = 1 To number
                Dim res As Resource = New Resource(i, String.Format("Resource {0}", i))
